...
const ROLE_CONFIG = {
  ...
  fuel_spv:{label:'Fuel SPV Operator',site:'Fuel Blending',scope:'Discharge, custody receive & transfer, barge loading, monitoring dan approval operasional',modules:['dashboard','shipment_monitor','bl_analysis','custody_transfer','cargo_calculator','discharge','marine_loading','validation']},
  ...
  it_admin:{label:'IT Administrator',site:'Enterprise',scope:'Akses seluruh site, master data, audit dan role management',modules:['dashboard','pre_inspection','loading','after_inspection','shipment_monitor','receiving','site_history','bl_analysis','custody_transfer','cargo_calculator','discharge','marine_loading','validation','admin']}
};
const NAV = [
  {label:'Overview',items:[['dashboard','▦','Live Dashboard']]},
...
const state = {
  role:'fuel_spv', page:'bl_analysis',
  inspections:[],
  shipments:[],
  discharge:[],
  cargoTransfers:[],
...
function badge(status){
  const m={IN_TRANSIT:['info','IN TRANSIT'],RECEIVED:['good','RECEIVED'],HOLD:['bad','HOLD / INVESTIGATE'],AWAITING_AFTER:['warn','WAIT AFTER INSPECTION'],PASS:['good','PASS'],COMPLETED:['good','COMPLETED'],IN_PROGRESS:['warn','IN PROGRESS']};
...
function setupRoles(){
  ...
  $('#roleSelect').onchange=e=>{state.role=e.target.value;state.page='dashboard';renderAll();};
}
function renderNav(){
...
function blAnalysisPage(){
  ...
  return `${pageTitle('BL Analysis Report','Rekonsiliasi dokumen loading port untuk membandingkan Bill of Lading, ship ullage figures dan independent surveyor figures sebelum data digunakan sebagai baseline custody discharge.','<div class="cargo-toolbar"><button class="btn" id="clearBlAnalysisBtn">Clear Analysis Data</button><button class="btn btn-primary" onclick="goPage(\'custody_transfer\')">Continue to Custody Transfer</button></div>')}
