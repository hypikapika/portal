...
/* =========================================================
   GLASSMORPHISM DASHBOARD VISUAL REFRESH — 2026-07-20
   Overrides dashboard pages visually without removing workflows.
   ========================================================= */
(function(){
  const dashboardCharts = {};
  function destroyDashboardCharts(){
    if(typeof Chart==='undefined') return;
    Object.keys(dashboardCharts).forEach(k=>{ try{ dashboardCharts[k].destroy(); }catch(e){} delete dashboardCharts[k]; });
  }
  function chart(id, config){
    if(typeof Chart==='undefined') return;
    const canvas=document.getElementById(id); if(!canvas) return;
    const ctx=canvas.getContext('2d');
    if(dashboardCharts[id]){ try{ dashboardCharts[id].destroy(); }catch(e){} }
    dashboardCharts[id]=new Chart(ctx, config);
  }
  function baseChartOptions(){
    return {
      responsive:true, maintainAspectRatio:false,
      plugins:{legend:{labels:{color:'#9fb0bd',boxWidth:10,usePointStyle:true,pointStyle:'circle'}},tooltip:{backgroundColor:'rgba(8,12,18,.92)',borderColor:'rgba(255,255,255,.09)',borderWidth:1,titleColor:'#fff',bodyColor:'#d9e1e8'}},
      scales:{
        x:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#8d9aa5',font:{family:'JetBrains Mono',size:10}}},
        y:{grid:{color:'rgba(255,255,255,.05)'},ticks:{color:'#8d9aa5',font:{family:'JetBrains Mono',size:10}}}
      }
    };
  }
  function compactDate(v){ return String(v||'—').split(',')[0]; }
  function lastNLabels(n,prefix='D'){ return Array.from({length:n},(_,i)=>`${prefix}${i+1}`); }
  function recentRequestVolumesBySite(){
    const sites=['RF','MAINTANK','MILL'];
    return sites.map(site=>siteOrders(site).reduce((a,o)=>a+Number(o.requestLiters||0),0)/1000);
  }
  function shipmentStatusCounts(){
    return {
      inTransit: state.shipments.filter(s=>s.status==='IN_TRANSIT').length,
      awaitingCR: state.shipments.filter(s=>s.status==='AWAITING_CR').length,
      hold: state.shipments.filter(s=>s.status==='HOLD').length,
      received: state.shipments.filter(s=>s.status==='RECEIVED').length
    };
  }
  function latestThroughputSeries(count=7){
    const rows=state.shipments.slice(0,count).reverse();
    return {
      labels: rows.map((s,i)=> s.id?.slice(-4) || `S${i+1}`),
      loading: rows.map(s=>Number(s.total||0)/1000),
      receiving: rows.map(s=>Number(s.validation?.received||0)/1000)
    };
  }
  function inventorySeries(){
    const rows=state.inventory.slice(0,5);
    return {
      labels: rows.map(r=>r.tank),
      book: rows.map(r=>Math.round(Number(r.bookStock||0)/1000)),
      sounding: rows.map(r=>Math.round(Number(r.soundingStock||0)/1000))
    };
  }
  function fleetStatusSeries(){
    const map={AVAILABLE:0,READY:0,ALLOCATED:0,NOT_READY:0};
    state.transportUnits.forEach(u=>{const s=String(u.status||'').toUpperCase().replaceAll(' ','_');if(map[s]==null) map.NOT_READY++; else map[s]++;});
    return map;
  }
  function pageMetrics(items){
    return `<div class="page-metrics">${items.map(i=>`<div class="metric-chip"><div class="label">${i.label}</div><div class="value">${i.value}</div><div class="sub">${i.sub||''}</div><div class="meter"><span style="width:${Math.max(6,Math.min(100,Number(i.pct||0)))}%"></span></div></div>`).join('')}</div>`;
  }
  function chartCard(id,title,sub,body,extra=''){ return `<div class="glass-chart-card ${extra}"><div class="glass-chart-head"><div><h3>${title}</h3><p>${sub}</p></div></div>${body}</div>`; }
  function statsList(rows){ return `<div class="chart-stat-list">${rows.map(r=>`<div class="chart-stat"><div><div class="stat-label">${r.label}</div><div class="stat-value ${r.cls||''}">${r.value}</div><div class="stat-note">${r.note||''}</div></div>${r.side?`<div class="stat-note" style="text-align:right">${r.side}</div>`:''}</div>`).join('')}</div>`; }
  function heroStrip(site,items){ return `${enterpriseHero(items.title,items.desc,site,items.action||'')}${pageMetrics(items.metrics)}`; }

  enterpriseDashboard = function(){
    const open=state.orders.filter(o=>!o.locked).length, ships=state.shipments.length, received=state.shipments.filter(s=>s.status==='RECEIVED').length, holds=state.shipments.filter(s=>s.status==='HOLD').length;
    const status=shipmentStatusCounts();
    return `${heroStrip('FB',{title:'Enterprise Operations Dashboard',desc:'Cross-role cockpit dengan visual distribusi request, progres shipment, inventory, customer receiving, dan workflow exception.',metrics:[
      {label:'Open Request',value:open,sub:'Belum inspection lock',pct:safePct(open,Math.max(open+received,1))},
      {label:'Active Shipment',value:ships,sub:'Semua custody chain',pct:safePct(status.inTransit+status.awaitingCR,Math.max(ships,1))},
      {label:'Receiving Done',value:received,sub:'Final reconciliation selesai',pct:safePct(received,Math.max(ships,1))},
      {label:'Hold / Alert',value:holds,sub:'Butuh review cepat',pct:safePct(holds,Math.max(ships,1))}
    ]})}
    <div class="dashboard-visual-grid triple">
      ${chartCard('ent-flow','Operational Throughput','Actual loading vs receiving terakhir','<div class="chart-stage compact"><canvas id="entFlowChart"></canvas></div><div class="visual-legend"><span class="l3">Loading KL</span><span class="l2">Receiving KL</span></div>')}
      ${chartCard('ent-request','Request Distribution','Volume request by customer site','<div class="chart-stage compact"><canvas id="entRequestChart"></canvas></div><div class="visual-legend"><span class="l1">RF</span><span class="l2">Maintank</span><span class="l3">Mill</span></div>')}
      ${chartCard('ent-status','Workflow Snapshot','Status ringkas seluruh shipment',statsList([
        {label:'In Transit',value:status.inTransit,note:'On-road delivery'},
        {label:'Awaiting CR',value:status.awaitingCR,note:'Finish-loading packet'},
        {label:'Received',value:status.received,note:'Customer confirmed'},
        {label:'Hold',value:status.hold,note:'Exception / investigation',cls:'text-bad'}
      ]))}
    </div>
    <div class="inventory-grid">${inventoryCards()}</div>
    <div class="layout-2"><div class="panel"><div class="panel-head"><div><div class="panel-title">Latest Cross-Site Requests</div><div class="panel-sub">RF, Maintank, dan Mill</div></div></div><div class="panel-body flush">${requestTableV3(state.orders.slice(0,10))}</div></div><div class="panel"><div class="panel-head"><div><div class="panel-title">Live Operational Timeline</div><div class="panel-sub">Latest workflow events</div></div></div><div class="panel-body">${dashboardTimeline(null)}</div></div></div>`;
  ...
  maintankDashboard = function(){
    const site='MAINTANK',c=requestStatusCounts(site),incoming=siteInTransit(site),received=siteReceived(site),empty=state.emptyUnitRecords.filter(x=>x.site==='MAINTANK').length;
    const rows=siteShipments(site);
    const reconOK=rows.filter(s=>s.validation?.volumePass).length;
    return `${heroStrip(site,{title:'Maintank Live Dashboard',desc:'Hanya menampilkan alur Fuel Blending ↔ Maintank: request, delivery, receiving, empty-unit sealing, dan evidence operational.',action:'<button class="btn btn-primary" onclick="goPage(\'maintank_requests\')">New Truck Request</button>',metrics:[
      {label:'Open Request',value:c.open,sub:'Editable until PASS',pct:safePct(c.open,Math.max(c.total,1))},
      {label:'Inbound Unit',value:incoming.length,sub:'From Fuel Blending',pct:safePct(incoming.length,Math.max(rows.length,1))},
      {label:'Receiving Done',value:received.length,sub:'Final result submitted',pct:safePct(received.length,Math.max(rows.length,1))},
      {label:'Empty Unit Seal',value:empty,sub:'Return empty controlled',pct:Math.min(100,empty*8)}
    ]})}
    <div class="dashboard-strip"><div class="strip-title">VISIBLE SCOPE</div><span class="strip-item">Fuel Blending</span><span class="strip-item">Maintank</span><span class="strip-item">No RF / Mill status</span></div>
    <div class="dashboard-visual-grid">
      ${chartCard('mt-delivery','Delivery vs Receiving','Jumlah unit menurut status akhir Maintank','<div class="chart-stage compact"><canvas id="mtDeliveryChart"></canvas></div>')}
      ${chartCard('mt-recon','Reconciliation Snapshot','Ringkasan validasi receiving Maintank',statsList([
        {label:'Inbound Packet',value:rows.length,note:'Linked with request'},
        {label:'Within Tolerance',value:reconOK,note:'Actual loading vs receiving',cls:'text-good'},
        {label:'Exceptions',value:rows.filter(s=>s.status==='HOLD').length,note:'Seal / sounding / volume',cls:'text-bad'}
      ]))}
    </div>
    <div class="layout-2"><div class="panel"><div class="panel-head"><div><div class="panel-title">Fuel Blending Delivery to Maintank</div><div class="panel-sub">Only shipments matching Maintank requests</div></div></div><div class="panel-body flush">${shipmentMiniTable(rows,true)}</div></div><div class="panel"><div class="panel-head"><div><div class="panel-title">Maintank Activity</div><div class="panel-sub">Request to receiving timeline</div></div></div><div class="panel-body">${dashboardTimeline(site)}</div></div></div>`;
  };

  rfDashboardV2 = function(){
    const site='RF',c=requestStatusCounts(site),incoming=siteInTransit(site),received=siteReceived(site),alerts=siteShipments(site).filter(s=>s.status==='HOLD').length;
    const rows=siteShipments(site);
    return `${heroStrip(site,{title:'Riau Fiber Live Dashboard',desc:'Request unit ke Fuel Blending, live loading status, physical arrival check, unloading detail, dan actual receiving.',action:'<button class="btn btn-primary" onclick="goPage(\'rf_requests_v2\')">New RF Request</button>',metrics:[
      {label:'Open Request',value:c.open,sub:'Duplicate active plate blocked',pct:safePct(c.open,Math.max(c.total,1))},
      {label:'Inbound Dispatch',value:incoming.length,sub:'Released by CR',pct:safePct(incoming.length,Math.max(rows.length,1))},
      {label:'Receiving Done',value:received.length,sub:'Unload area & operator recorded',pct:safePct(received.length,Math.max(rows.length,1))},
      {label:'Security Alert',value:alerts,sub:'Seal / identity / damage',pct:safePct(alerts,Math.max(rows.length,1))}
    ]})}
    <div class="dashboard-visual-grid">
      ${chartCard('rf-flow','RF Request Flow','Request, transit, and completed receiving','<div class="chart-stage compact"><canvas id="rfFlowChart"></canvas></div>')}
      ${chartCard('rf-health','RF Exception Focus','Operational quick review',statsList([
        {label:'Awaiting Receiving',value:rows.filter(s=>s.status==='AWAITING_RECEIVING').length,note:'Physical arrival matched'},
        {label:'Delivered Today',value:incoming.length,note:'Dispatch notification visible'},
        {label:'Hold / Investigate',value:alerts,note:'Potential custody risk',cls:'text-bad'}
      ]))}
    </div>
    <div class="layout-2"><div class="panel"><div class="panel-head"><div><div class="panel-title">RF Request & Loading Status</div><div class="panel-sub">Live request chain</div></div></div><div class="panel-body flush">${requestTableV3(siteOrders(site).slice(0,12),{showSite:false,actions:true})}</div></div><div class="panel"><div class="panel-head"><div><div class="panel-title">RF Delivery Stream</div><div class="panel-sub">Latest Fuel Blending dispatch</div></div></div><div class="panel-body flush">${shipmentMiniTable(rows.slice(0,10),true)}</div></div></div>`;
  };

  millDashboard = function(){
    const site='MILL',c=requestStatusCounts(site),incoming=siteInTransit(site),received=siteReceived(site),q=state.qualityChecks.filter(x=>x.site==='MILL').slice(0,1)[0];
    const rows=siteShipments(site);
    return `${heroStrip(site,{title:'Mill Live Dashboard',desc:'Request order, surat jalan validation, sample check, dan cargo color evidence untuk custody receiving Mill.',action:'<button class="btn btn-primary" onclick="goPage(\'mill_requests\')">New Mill Order</button>',metrics:[
      {label:'Open Order',value:c.open,sub:'Mill requests',pct:safePct(c.open,Math.max(c.total,1))},
      {label:'Inbound Unit',value:incoming.length,sub:'From Fuel Blending',pct:safePct(incoming.length,Math.max(rows.length,1))},
      {label:'Receiving Done',value:received.length,sub:'Final volume reconciled',pct:safePct(received.length,Math.max(rows.length,1))},
      {label:'Cargo Color',value:q?.colorStatus||'N/C',sub:'Visual screening only',pct:q?.colorStatus==='BAIK'?84:35}
    ]})}
    <div class="dashboard-visual-grid">
      ${chartCard('mill-flow','Mill Operational Flow','Order → inbound → receiving','<div class="chart-stage compact"><canvas id="millFlowChart"></canvas></div>')}
      ${chartCard('mill-quality','Latest Mill Quality Check','Visual color screening is not a lab test',q?qualityResultView(q):'<div class="empty-note">Belum ada cargo sample check.</div>')}
    </div>
    <div class="layout-2"><div class="panel"><div class="panel-head"><div><div class="panel-title">Mill Order Queue</div><div class="panel-sub">Surat jalan and sample requirements</div></div></div><div class="panel-body flush">${requestTableV3(siteOrders(site),{showSite:false,actions:true})}</div></div><div class="panel"><div class="panel-head"><div><div class="panel-title">Mill Delivery Stream</div><div class="panel-sub">Latest shipments to Mill</div></div></div><div class="panel-body flush">${shipmentMiniTable(rows.slice(0,10),true)}</div></div></div>`;
  };

  transportDashboard = function(){
    const counts=fleetStatusSeries();
    return `${heroStrip('FB',{title:'Transport Live Dashboard',desc:'Fleet readiness untuk mendukung request dari Maintank, Riau Fiber, dan Mill. Status unit memisahkan available, ready, allocated, dan workshop.',action:'<button class="btn btn-primary" onclick="goPage(\'transport_fleet\')">Manage Fleet</button>',metrics:[
      {label:'Available',value:counts.AVAILABLE,sub:'Can be requested',pct:counts.AVAILABLE*12},
      {label:'Ready',value:counts.READY,sub:'Pre-checked & clean',pct:counts.READY*12},
      {label:'Allocated',value:counts.ALLOCATED,sub:'Serving active request',pct:counts.ALLOCATED*12},
      {label:'Workshop',value:counts.NOT_READY,sub:'Repair / unavailable',pct:counts.NOT_READY*12}
    ]})}
    <div class="dashboard-visual-grid">
      ${chartCard('fleet-status','Fleet Status Mix','Availability of transport assets','<div class="chart-stage compact"><canvas id="fleetStatusChart"></canvas></div>')}
      ${chartCard('fleet-note','Operational Notes','Snapshot status dari seluruh armada',statsList([
        {label:'Available',value:counts.AVAILABLE,note:'Can be assigned now'},
        {label:'Ready',value:counts.READY,note:'Ready with checks complete',cls:'text-good'},
        {label:'Allocated',value:counts.ALLOCATED,note:'Attached to active requests'},
        {label:'Workshop',value:counts.NOT_READY,note:'Not ready / maintenance',cls:'text-warn'}
      ]))}
    </div>${fleetTable()}`;
  };

  fuelOpsDashboard = function(){
    const requests=state.orders.filter(o=>!o.locked),pre=state.inspections.filter(i=>i.result==='PASS').length,loading=state.shipments.filter(s=>['AWAITING_AFTER','AWAITING_CR'].includes(s.status)),alerts=state.shipments.filter(s=>s.status==='HOLD');
    return `${heroStrip('FB',{title:'Fuel Blending Live Dashboard',desc:'Request dari customer masuk ke satu inbox. Operator menjalankan before inspection → loading → finish loading inspection dengan live notifikasi.',action:'<button class="btn btn-primary" onclick="goPage(\'request_inbox\')">Open Request Inbox</button>',metrics:[
      {label:'Waiting Request',value:requests.length,sub:'RF + Maintank + Mill',pct:safePct(requests.length,Math.max(state.orders.length,1))},
      {label:'Inspection PASS',value:pre,sub:'Locked order records',pct:safePct(pre,Math.max(state.inspections.length,1))},
      {label:'Loading in Process',value:loading.length,sub:'Field or finish loading',pct:safePct(loading.length,Math.max(state.shipments.length,1))},
      {label:'Hold Alert',value:alerts.length,sub:'Condition / seal exception',pct:safePct(alerts.length,Math.max(state.shipments.length,1))}
    ]})}
    <div class="dashboard-visual-grid triple">
      ${chartCard('fb-request-mix','Customer Request Mix','Request volume by destination','<div class="chart-stage compact"><canvas id="fbRequestMixChart"></canvas></div>')}
      ${chartCard('fb-progress','Inspection & Loading Progress','Current work in pipeline','<div class="chart-stage compact"><canvas id="fbProgressChart"></canvas></div>')}
      ${chartCard('fb-news','Bulletin Snapshot','Superior, maintenance, project, and safety', statsList([
        {label:'News Published',value:state.bulletins.length,note:'Operational & safety updates'},
        {label:'Open Requests',value:requests.length,note:'Need next action'},
        {label:'Current Holds',value:alerts.length,note:'Review immediately',cls:'text-bad'}
      ]))}
    </div>
    <div class="layout-2"><div class="panel"><div class="panel-head"><div><div class="panel-title">Incoming Customer Requests</div><div class="panel-sub">Sorted by request timestamp</div></div></div><div class="panel-body flush">${requestTableV3(state.orders.filter(o=>!o.locked).slice(0,12))}</div></div><div class="panel"><div class="panel-head"><div><div class="panel-title">Superior & Safety Bulletin</div><div class="panel-sub">Maintenance, project and safety notifications</div></div></div><div class="panel-body">${bulletinCards('fuel_field')}</div></div></div>`;
  };

  crDashboardV2 = function(){
    const q=state.shipments.filter(s=>s.status==='AWAITING_CR'),transit=state.shipments.filter(s=>s.status==='IN_TRANSIT'),stock=state.inventory.reduce((a,t)=>a+Number(t.soundingStock||0),0),varSum=state.inventory.reduce((a,t)=>a+Math.abs(Number(t.soundingStock||0)-Number(t.bookStock||0)),0);
    return `${heroStrip('FB',{title:'Control Room Dashboard',desc:'Final loading verification, dispatch control, dan stock inventory monitoring untuk mendukung planning shipment.',action:'<button class="btn btn-primary" onclick="goPage(\'cr_loading_v2\')">Open CR Queue</button>',metrics:[
      {label:'CR Queue',value:q.length,sub:'Awaiting verification',pct:safePct(q.length,Math.max(state.shipments.length,1))},
      {label:'In Transit',value:transit.length,sub:'Verified and released',pct:safePct(transit.length,Math.max(state.shipments.length,1))},
      {label:'Total Sounding',value:fmt(stock)+' L',sub:'Across configured tanks',pct:62},
      {label:'Abs Variance',value:fmt(varSum)+' L',sub:'Book vs sounding',pct:Math.min(100,varSum/500)}
    ]})}
    <div class="dashboard-visual-grid">
      ${chartCard('cr-inventory','Inventory Control','Book stock vs physical sounding by tank','<div class="chart-stage"><canvas id="crInventoryChart"></canvas></div>')}
      ${chartCard('cr-queue','CR Queue Focus','Dispatch verification snapshot',statsList([
        {label:'Awaiting CR',value:q.length,note:'After-loading packet ready'},
        {label:'Dispatched',value:transit.length,note:'On route to customer',cls:'text-good'},
        {label:'Inventory Tanks',value:state.inventory.length,note:'Tracked in stock control'},
        {label:'Bulletins',value:state.bulletins.length,note:'Maintenance / project / safety'}
      ]))}
    </div>
    <div class="inventory-grid">${inventoryCards()}</div><div class="layout-2"><div class="panel"><div class="panel-head"><div><div class="panel-title">CR Verification Queue</div><div class="panel-sub">All customer sites</div></div></div><div class="panel-body flush">${shipmentMiniTable(q)}</div></div><div class="panel"><div class="panel-head"><div><div class="panel-title">Operations Bulletin</div><div class="panel-sub">SPV maintenance, projects and safety news</div></div></div><div class="panel-body">${bulletinCards('cr_operator')}</div></div></div>`;
  };

  spvDashboardV2 = function(){
    const complaints=state.complaints.filter(c=>c.status!=='CLOSED'),schedules=state.shipmentSchedules.filter(x=>x.status!=='COMPLETED'),maint=state.maintenance.filter(x=>x.status!=='COMPLETED'),projects=state.projects.filter(x=>x.status!=='COMPLETED');
    return `${heroStrip('FB',{title:'Fuel SPV Dashboard',desc:'Supervision hub untuk shipment planning berbasis inventory, customer complaints, preventive maintenance, project control, dan port custody tools.',metrics:[
      {label:'Open Complaints',value:complaints.length,sub:'Need superior feedback',pct:safePct(complaints.length,Math.max(state.complaints.length,1))},
      {label:'Shipment Plans',value:schedules.length,sub:'Linked with CR inventory',pct:safePct(schedules.length,Math.max(schedules.length+maint.length+projects.length,1))},
      {label:'Open PM',value:maint.length,sub:'Preventive work schedule',pct:safePct(maint.length,Math.max(maint.length+projects.length,1))},
      {label:'Active Project',value:projects.length,sub:'Operational improvement',pct:safePct(projects.length,Math.max(maint.length+projects.length,1))}
    ]})}
    <div class="dashboard-visual-grid triple">
      ${chartCard('spv-plan','Planning & Maintenance','Open work balance','<div class="chart-stage compact"><canvas id="spvWorkChart"></canvas></div>')}
      ${chartCard('spv-throughput','Shipment Throughput','Loading vs receiving latest chain','<div class="chart-stage compact"><canvas id="spvThroughputChart"></canvas></div>')}
      ${chartCard('spv-focus','Priority Focus','Immediate supervisor attention',statsList([
        {label:'Open Complaints',value:complaints.length,note:'Customer issue handling',cls:complaints.length?'text-warn':''},
        {label:'Hold Shipments',value:state.shipments.filter(s=>s.status==='HOLD').length,note:'Operational exception',cls:'text-bad'},
        {label:'Plans on Hold',value:schedules.filter(s=>s.status==='HOLD').length,note:'Stock insufficient'},
        {label:'News Published',value:state.bulletins.length,note:'Ops communication'}
      ]))}
    </div>
    <div class="inventory-grid">${inventoryCards()}</div><div class="layout-2"><div class="panel"><div class="panel-head"><div><div class="panel-title">Open Complaints</div><div class="panel-sub">Customer feedback requiring action</div></div></div><div class="panel-body">${complaintList(false,6)}</div></div><div class="panel"><div class="panel-head"><div><div class="panel-title">Current Shipment Plans</div><div class="panel-sub">Stock-aware scheduling</div></div></div><div class="panel-body">${scheduleList(6)}</div></div></div>`;
  };

  function mountGlassDashboardCharts(){
    destroyDashboardCharts();
    const common=baseChartOptions();
    if(state.page==='enterprise_dashboard'){
      const s=latestThroughputSeries();
      chart('entFlowChart',{type:'line',data:{labels:s.labels,datasets:[{label:'Loading KL',data:s.loading,borderColor:'#f0a43a',backgroundColor:'rgba(240,164,58,.18)',fill:true,tension:.35,pointRadius:3},{label:'Receiving KL',data:s.receiving,borderColor:'#50d5c7',backgroundColor:'rgba(80,213,199,.14)',fill:true,tension:.35,pointRadius:3}]},options:common});
      chart('entRequestChart',{type:'doughnut',data:{labels:['RF','Maintank','Mill'],datasets:[{data:recentRequestVolumesBySite(),backgroundColor:['#5ea7ff','#50d5c7','#f0a43a'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#9fb0bd',usePointStyle:true}}},cutout:'68%'}});
    }
    if(state.page==='maintank_dashboard'){
      const rows=siteShipments('MAINTANK');
      chart('mtDeliveryChart',{type:'bar',data:{labels:['Inbound','Received','Hold'],datasets:[{label:'Units',data:[siteInTransit('MAINTANK').length,siteReceived('MAINTANK').length,rows.filter(s=>s.status==='HOLD').length],backgroundColor:['rgba(94,167,255,.78)','rgba(66,211,146,.78)','rgba(255,98,90,.78)'],borderRadius:8}]},options:common});
    }
    if(state.page==='rf_dashboard_v2'){
      chart('rfFlowChart',{type:'bar',data:{labels:['Open Request','In Transit','Await Receiving','Completed'],datasets:[{label:'RF Units',data:[requestStatusCounts('RF').open,siteInTransit('RF').length,siteShipments('RF').filter(s=>s.status==='AWAITING_RECEIVING').length,siteReceived('RF').length],backgroundColor:['rgba(94,167,255,.78)','rgba(80,213,199,.78)','rgba(255,209,102,.82)','rgba(66,211,146,.82)'],borderRadius:8}]},options:common});
    }
    if(state.page==='mill_dashboard'){
      chart('millFlowChart',{type:'bar',data:{labels:['Open','Inbound','Received'],datasets:[{label:'Mill',data:[requestStatusCounts('MILL').open,siteInTransit('MILL').length,siteReceived('MILL').length],backgroundColor:['rgba(94,167,255,.8)','rgba(80,213,199,.8)','rgba(66,211,146,.8)'],borderRadius:8}]},options:common});
    }
    if(state.page==='transport_dashboard'){
      const f=fleetStatusSeries();
      chart('fleetStatusChart',{type:'doughnut',data:{labels:['Available','Ready','Allocated','Workshop'],datasets:[{data:[f.AVAILABLE,f.READY,f.ALLOCATED,f.NOT_READY],backgroundColor:['#5ea7ff','#42d392','#50d5c7','#f0a43a'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#9fb0bd',usePointStyle:true}}},cutout:'68%'}});
    }
    if(state.page==='fuel_ops_dashboard'){
      chart('fbRequestMixChart',{type:'doughnut',data:{labels:['RF','Maintank','Mill'],datasets:[{data:recentRequestVolumesBySite(),backgroundColor:['#5ea7ff','#50d5c7','#f0a43a'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#9fb0bd',usePointStyle:true}}},cutout:'66%'}});
      chart('fbProgressChart',{type:'bar',data:{labels:['Open Requests','Inspection PASS','Loading','Hold'],datasets:[{label:'Count',data:[state.orders.filter(o=>!o.locked).length,state.inspections.filter(i=>i.result==='PASS').length,state.shipments.filter(s=>['AWAITING_AFTER','AWAITING_CR'].includes(s.status)).length,state.shipments.filter(s=>s.status==='HOLD').length],backgroundColor:['rgba(94,167,255,.8)','rgba(66,211,146,.82)','rgba(240,164,58,.82)','rgba(255,98,90,.82)'],borderRadius:8}]},options:common});
    }
    if(state.page==='cr_dashboard_v2'){
      const inv=inventorySeries();
      chart('crInventoryChart',{type:'bar',data:{labels:inv.labels,datasets:[{label:'Book (KL)',data:inv.book,backgroundColor:'rgba(94,167,255,.72)',borderRadius:8},{label:'Sounding (KL)',data:inv.sounding,backgroundColor:'rgba(80,213,199,.78)',borderRadius:8}]},options:common});
    }
    if(state.page==='spv_dashboard_v2'){
      chart('spvWorkChart',{type:'doughnut',data:{labels:['Complaints','Schedules','Maintenance','Projects'],datasets:[{data:[state.complaints.filter(c=>c.status!=='CLOSED').length,state.shipmentSchedules.filter(x=>x.status!=='COMPLETED').length,state.maintenance.filter(x=>x.status!=='COMPLETED').length,state.projects.filter(x=>x.status!=='COMPLETED').length],backgroundColor:['#ff625a','#5ea7ff','#f0a43a','#50d5c7'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#9fb0bd',usePointStyle:true}}},cutout:'66%'}});
      const s=latestThroughputSeries();
      chart('spvThroughputChart',{type:'line',data:{labels:s.labels,datasets:[{label:'Loading KL',data:s.loading,borderColor:'#5ea7ff',backgroundColor:'rgba(94,167,255,.16)',fill:true,tension:.35,pointRadius:3},{label:'Receiving KL',data:s.receiving,borderColor:'#50d5c7',backgroundColor:'rgba(80,213,199,.12)',fill:true,tension:.35,pointRadius:3}]},options:common});
    }
  }

  const _renderPageGlass = renderPage;
  renderPage = function(){ _renderPageGlass(); mountGlassDashboardCharts(); };
  renderAll();
})();
